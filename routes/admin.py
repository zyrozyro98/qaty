...
from models import User, Product, Order, Market
        ...
        for demo_user in demo_users:
            if not User.query.filter_by(username=demo_user['username']).first():
                user = User(
                    ...
                    password=hashlib.sha256(demo_user['password'].encode()).hexdigest(),
                    full_name=demo_user['full_name'],
                    email=demo_user['email'],
                    phone=demo_user['phone'],
                    role=demo_user['role'],
                    store_name=demo_user.get('store_name', ''),
                    vehicle_type=demo_user.get('vehicle_type', ''),
                    wallet_balance=demo_user.get('wallet_balance', 0.0),
                    is_active=True
                )
                db.session.add(user)
